...
@pytest.mark.asyncio
async def test_keycloak_openid_init(env):
...
async def test_token(oid_with_credentials: Tuple[KeycloakOpenID, str, str]):
    ...
    # Test refresh token
    token = await oid.refresh_token(refresh_token=token["refresh_token"])
    assert token == {
        "access_token": mock.ANY,
        "expires_in": 300,
        "id_token": mock.ANY,
...
async def test_certs(oid: KeycloakOpenID):
    ...
    :type oid: KeycloakOpenID
    """
    assert len((await oid.certs())["keys"]) == 2
...
async def test_entitlement(
    ...
    token = await oid.token(username=username, password=password)
    resource_server_id = (await admin.get_client_authz_resources(
...
async def test_introspect(oid_with_credentials: Tuple[KeycloakOpenID, str, str]):
    ...
    assert (await oid.introspect(token=token["access_token"]))["active"]
    assert await oid.introspect(
...
async def test_load_authorization_config(oid_with_credentials_authz: Tuple[KeycloakOpenID, str, str]):
    ...
    await oid.load_authorization_config(path=os.path.join(dir_path, "data/authz_settings.json"))
...
async def test_uma_permissions(oid_with_credentials_authz: Tuple[KeycloakOpenID, str, str]):
    ...
    assert (await oid.uma_permissions(token=token["access_token"]))[0]["rsname"] == "Default Resource"
