...
async def test_keycloak_admin_init(env):
    ...
    await admin.init_token()
    assert admin.connection.server_url == f"http://{env.KEYCLOAK_HOST}:{env.KEYCLOAK_PORT}", admin.connection.server_url
    ...
    secret = await admin.generate_client_secrets(client_id=await admin.get_client_id("authz-client"))
    ...
    await admin2.init_token()
    assert admin2.connection.token
    ...
    await keycloak_connection.init_token()
...
async def test_groups(admin: KeycloakAdmin, user: str):
    ...
    with pytest.raises(KeycloakGetError) as err:
        await admin.get_group_by_path(path="/main-group/subgroup-2/subsubgroup-1/test")
    assert err.match('404: b\'{"error":"Group path does not exist"}\'')

    res = await admin.get_group_by_path(path="/main-group/subgroup-2/subsubgroup-1")
    assert res is not None, res
    assert res["id"] == subsubgroup_id_1
    ...
    assert res == dict(), res
    assert (await admin.get_group(group_id=subgroup_id_2))["name"] == "new-subgroup-2"
    ...
    assert res == dict(), res
    assert len(await admin.get_groups()) == 0
...
async def test_clients(admin: KeycloakAdmin, realm: str):
    ...
    assert {x["name"] for x in clients} == {"${client_admin-cli}", "${client_security-admin-console}",
                                            "${client_account-console}", "${client_broker}", "${client_account}",
                                            "${client_realm-management}"}, clients
    ...
    mapper = (await admin.get_mappers_from_client(client_id=client_id))[0]
    with pytest.raises(KeycloakPutError) as err:
    ...
    res = await admin.get_client_authz_resource(client_id=auth_client_id, resource_id=test_resource_id)
    ...
    await admin.delete_client_authz_resource(client_id=auth_client_id, resource_id=temp_resource_id)
    with pytest.raises(KeycloakGetError) as err:
        await admin.get_client_authz_resource(client_id=auth_client_id, resource_id=temp_resource_id)
    ...
    role_id = (await admin.get_realm_role(role_name="offline_access"))["id"]
    ...
    res2 = await admin.get_client_authz_policy(client_id=auth_client_id, policy_id=res["id"])
    assert res["id"] == res2["id"]
    await admin.delete_client_authz_policy(client_id=auth_client_id, policy_id=res["id"])
    with pytest.raises(KeycloakGetError) as err:
        await admin.get_client_authz_policy(client_id=auth_client_id, policy_id=res["id"])
    assert err.match("404: b''")
...
async def test_realm_roles(admin: KeycloakAdmin, realm: str):
    ...
    assert res == dict(), res
    assert (await admin.get_user(user_id=user_id))["username"] in [
        x["username"] for x in await admin.get_realm_role_members(role_name="offline_access")
    ]
    assert (await admin.get_user(user_id=user_id))["username"] in [
        x["username"] for x in await admin.get_realm_role_members(role_name="test-realm-role-update")
    ]
...
async def test_client_scope_realm_roles(admin: KeycloakAdmin, realm: str):
    ...
    res = await admin.delete_realm_roles_of_client_scope(
        client_id=client_id, roles=[await admin.get_realm_role(role_name="offline_access")]
    )
    assert res == dict(), res
    roles = await admin.get_realm_roles_of_client_scope(client_id=client_id)
    assert len(roles) == 1
    ...
    res = await admin.delete_realm_roles_of_client_scope(
        client_id=client_id, roles=[await admin.get_realm_role(role_name="test-realm-role")]
    )
    assert res == dict(), res
    roles = await admin.get_realm_roles_of_client_scope(client_id=client_id)
    assert len(roles) == 0
...
@pytest.mark.asyncio
async def test_client_default_client_scopes(admin: KeycloakAdmin, realm: str, client: str):
    ...
    default_client_scopes = await admin.get_client_default_client_scopes(client_id)
    ...
    new_client_scope_id = await admin.create_client_scope(new_client_scope, skip_exists=False)
    new_default_client_scope_data = {
        "realm": realm,
        "client": client_id,
        "clientScopeId": new_client_scope_id,
    }
    await admin.add_client_default_client_scope(
    ...
    await admin.delete_client_default_client_scope(client_id, new_client_scope_id)
    default_client_scopes = await admin.get_client_default_client_scopes(client_id)
    assert len(default_client_scopes) == 5, default_client_scopes


@pytest.mark.asyncio
async def test_client_optional_client_scopes(admin: KeycloakAdmin, realm: str, client: str):
    ...
    optional_client_scopes = await admin.get_client_optional_client_scopes(client_id)
    ...
    await admin.add_client_optional_client_scope(
    ...
    await admin.delete_client_optional_client_scope(client_id, new_client_scope_id)
    optional_client_scopes = await admin.get_client_optional_client_scopes(client_id)
    assert len(optional_client_scopes) == 4, optional_client_scopes
...
async def test_client_roles(admin: KeycloakAdmin, client: str):
    ...
    assert res == dict()
    assert (await admin.get_client_role(client_id=client, role_name="client-role-test-update"))[
        "composite"
    ]
...
async def test_enable_token_exchange(admin: KeycloakAdmin, realm: str):
    ...
    token_exchange_permission_id = (await admin.get_client_management_permissions(
    ...
    client_policy_id = (await admin.create_client_authz_client_policy(
        payload={
            "type": "client",
            "logic": "POSITIVE",
    ...
    permission_name = (await admin.get_client_authz_scope_permission(
...
async def test_get_sessions(admin: KeycloakAdmin):
    ...
    :type admin: KeycloakAdmin
    """
    sessions = await admin.get_sessions(user_id=await admin.get_user_id(username=admin.connection.username))
...
async def test_auth_flows(admin: KeycloakAdmin, realm: str):
    ...
    assert err.match('400: b\'{"error":"Unrecognized field')
    payload = (await admin.get_authentication_flow_executions(flow_alias="test-create"))[0]
    payload["displayName"] = "test"
    res = await admin.update_authentication_flow_executions(payload=payload, flow_alias="test-create")
    assert res

    exec_id = (await admin.get_authentication_flow_executions(flow_alias="test-create"))[0]["id"]
...
async def test_client_scopes(admin: KeycloakAdmin, realm: str):
    ...
    assert (await admin.get_client_scope(client_scope_id=res))["name"] == "test-scope-update"
    ...
    # Test update mapper
    test_mapper = (await admin.get_mappers_from_client_scope(client_scope_id=res))[0]
    with pytest.raises(KeycloakPutError) as err:
    ...
    assert res_update == dict()
    assert (
            (await admin.get_mappers_from_client_scope(client_scope_id=res))[0]["config"]["user.attribute"]
            == "test"
    )
...
async def test_components(admin: KeycloakAdmin, realm: str):
    ...
    assert res
    assert (await admin.get_component(component_id=res))["name"] == "Test Component"
    ...
    assert res_upd == dict()
    assert (await admin.get_component(component_id=res))["name"] == "Test Component Update"
...
async def test_keys(admin: KeycloakAdmin, realm: str):
    ...
    """
    admin.realm_name = realm
    assert set((await admin.get_keys())["active"].keys()) == {"AES", "HS256", "RS256", "RSA-OAEP"}
    assert {k["algorithm"] for k in (await admin.get_keys())["keys"]} == {
        "HS256",
        "RSA-OAEP",
        "AES",
        "RS256",
...
@pytest.mark.asyncio
async def test_auto_refresh(admin_frozen: KeycloakAdmin, realm: str):
    ...
    with freezegun.freeze_time("2023-02-25 10:10:00"):
        ...
        await admin.connection.get_token()
...
@pytest.mark.asyncio
async def test_clear_keys_cache(realm: str, admin: KeycloakAdmin) -> None:
    ...
    """
    admin.realm_name = realm
    res = await admin.clear_keys_cache()
    assert res == {}


@pytest.mark.asyncio
async def test_clear_realm_cache(realm: str, admin: KeycloakAdmin) -> None:
    ...
    """
    admin.realm_name = realm
    res = await admin.clear_realm_cache()
    assert res == {}
...
async def test_clear_user_cache(realm: str, admin: KeycloakAdmin) -> None:
    ...
    """
    admin.realm_name = realm
    res = await admin.clear_user_cache()
    assert res == {}


@pytest.mark.asyncio
async def test_initial_access_token(
    ...
    :type oid_with_credentials: Tuple[KeycloakOpenID, str, str]
    """
    res = await admin.create_initial_access_token(2, 3)
    assert "token" in res
    assert res["count"] == 2
